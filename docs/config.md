...
Sign up for Epic's SMART on FHIR credentials with the instructions here: https://fhir.epic.com/Documentation?docId=oauth2. Your client's ID and secret must be joined together with a `:` and provided to AuthN as a single variable.
